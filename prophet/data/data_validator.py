...
class StockDataValidator:
    ...
    def validate(self, symbol, history: pd.DataFrame):
        if self.start_date is not None:
            history = history[history['Date'] >= self.start_date]

        summary = {}
        for check in self.check_list:
            description, invalid_records = check(symbol, history)
            summary[description] = invalid_records
        return summary

    @staticmethod
    def __def_check(f, *args):
        def g(symbol, history):
            symbol, marks = f(symbol, history, *args)
            return symbol, history[marks]['Date']
        return g

    @staticmethod
    def __check_na(symbol, history: pd.DataFrame, column):
        return '{} = NA'.format(column), history[column].isna()

    @staticmethod
    def __check_neg(symbol, history: pd.DataFrame, column):
        return '{} <= 0'.format(column), history[column] <= 0

    @staticmethod
    def __check_ge(symbol, history: pd.DataFrame, column1, column2):
        return '{} > {}'.format(column1, column2), history[column1] > history[column2]

    @staticmethod
    def __check_oob(symbol, history: pd.DataFrame, column1, column2, column3):
        return '{} OOB [{}, {}]'.format(column1, column2, column3),\
            ~history[column1].between(history[column2], history[column3])

    @staticmethod
    def __check_jump(symbol, history: pd.DataFrame, column):
        threshold = 0.22 if symbol[0] == '3' else 0.11
